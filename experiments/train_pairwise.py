...
def main():
    ...
    thresholds = [i / 20 for i in range(5, 16)]
    l2_grid = (0.1, 1.0, 10.0)
    scored = []
    for l2 in l2_grid:
        candidate = PairwisePreferenceModel.fit(train, l2=l2)
        for threshold in thresholds:
            accuracy = (sum((candidate.predict_proba(r) >= threshold) == bool(r["preferred"])
                            for r in valid) / len(valid)) if valid else 0.0
            scored.append((accuracy, -l2, -threshold, candidate, threshold))
    _, _, _, model, threshold = max(scored, key=lambda item: (item[0], item[1], item[2]))
