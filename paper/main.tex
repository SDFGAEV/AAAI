...
Minecraft has emerged as a premier testbed for open-world agent research. Voyager~\cite{voyager2023} introduced executable code skill libraries with automatic curricula. DEPS~\cite{deps2023} refined interactive planning with hierarchical task decomposition. MineDojo~\cite{minedojo2022} contributed a large-scale benchmark with programmatic tasks. JARVIS-1~\cite{jarvis2024} demonstrated multimodal memory for long-horizon execution. These systems generate knowledge but apply no admission control---all generated knowledge is stored and reused without gating.

Subsequent work introduced consolidation mechanisms: parametric approaches train LoRA adapters from experience trajectories with heuristic scoring; rule-based curators check format validity and execution consistency; curriculum-guided methods select experiences for continual world-model learning; multi-axis transfer analysis decomposes similarity for cross-task reuse. \textbf{The structural commonality uniting every prior approach is that the gate evaluates intrinsic properties---the knowledge itself or its source episode. No prior gate measures the causal effect of reuse in a new context.} \textsc{C-ACT} addresses this gap by making admission a first-class decision with its own measurement apparatus.

\subsection{Causal Methods for Agent Decision-Making}

A growing body of work applies causal reasoning to agent memory and decision-making. CMI (Causal Memory Intervention) estimates the causal effect of individual dialogue turns on downstream answer quality, selecting which conversation segments to include in the LLM prompt at inference time. CausalFlow performs step-level attribution in reasoning and tool-use chains, identifying which specific action caused a failure for targeted repair. WISE constructs causal event graphs over spatial, temporal, and goal-oriented knowledge to guide retrieval scheduling. These methods share a common intervention target: they modify \emph{prompt composition}---deciding which text appears in the LLM's context window. \textsc{C-ACT} intervenes at a different level: it governs whether retrieved knowledge---which may or may not reach the prompt---is permitted to shape the agent's \emph{behavior}. The outcome metric also differs: prompt-level causal methods optimize for answer quality in single-turn or short-horizon tasks; \textsc{C-ACT} optimizes for task success and harm avoidance over multi-step embodied trajectories. Furthermore, \textsc{C-ACT} enforces \emph{safety constraints} that are absent from prompt-level selection: a knowledge item that improves expected answer quality could still cause harmful behavior (e.g., suggesting a dangerous action), and \textsc{C-ACT}'s harm posterior explicitly guards against this.

\subsection{Bayesian Decision-Making and Risk Calibration}

Bayesian methods have been successfully applied to sequential agent decisions, including bandit-based tool selection (AEL uses Thompson sampling to choose among learned tools) and confidence-calibrated planning (where capability estimates are used to select feasible subgoals). \textsc{C-ACT} differs from these approaches in two ways. First, its decision is \emph{binary} (admit or reject) rather than a ranking or selection among alternatives---the knowledge items compete only against the base policy, not against each other. Second, \textsc{C-ACT} jointly models uplift and harm, rather than optimizing for expected success alone. The TrustGate's calibration procedure connects to conformal prediction~\cite{angelopoulos2021}: thresholds are selected from a held-out calibration set by maximizing coverage subject to a finite-sample risk bound computed via the 90\% binomial confidence upper bound. However, \textsc{C-ACT} uses batch calibration rather than online conformal inference: thresholds are fixed after calibration and applied without update during evaluation, trading adaptivity for computational simplicity and auditability.
...
To achieve principled knowledge admission, \textsc{C-ACT} follows a seven-stage pipeline at each decision point:~\textbf{\ding{182}}~Contract filtering rejects knowledge whose preconditions are unmet or whose safety boundaries are violated;~\textbf{\ding{183}}~temporal decay shrinks stale evidence toward priors;~\textbf{\ding{184}}~Bayesian evidence computes $\pi_u$ and $h_u^+$ from three Beta posteriors;~\textbf{\ding{185}}~a candidate chain is built and ranked;~\textbf{\ding{186}}~pairwise interaction checking detects conflicts and promotes synergies;~\textbf{\ding{187}}~the four-condition gate admits or rejects; and~\textbf{\ding{188}}~active logging or Thompson probing may override the decision during accumulation phases. Figure~\ref{fig:architecture} shows the system architecture and Algorithm~\ref{alg:cact} provides the complete procedure.
